...
class Dream3dMicrotextureAnalysis(Frame):
    ...
    def initUI(self):
        ...
        frame2 = Frame(self)
        frame2.pack(fill=X)
        lbl2 = Label(frame2, text="Minimum MTR Size, um^2:", width=self.lw, anchor='e', justify=RIGHT, font=('Arial', '9','bold'), background='#f2f2f2')
        ...
        frame4 = Frame(self)
        frame4.pack(fill=X)
        lbl4 = Label(frame4, text="Stress Axis Direction:", width=self.lw, anchor='e', justify=RIGHT, font=('Arial', '9','bold'), background='#f2f2f2')
        ...
        self.note = StringVar()
        self.note.set("* The GUI may appear to freeze during this analysis, but it's usually still working in the background. Please be patient.")
    ...
    def onSubmit(self):
        ...
        if len(self.file_paths) > 0:
            ...
            response = messagebox.showwarning(title='Warning', message="No Dream3D file(s) were located. Verify path directory. Are you sure you want to exit?", type='yesno')
            if response == 'yes':
                self.quit()
        ...
        if len(self.file_paths):
            ...
            for n, fid in enumerate(d3d_paths):
                if os.path.exists(fid):
                    ...
                    data = np.c_[d3d['mtr_sizes'], d3d['mtr_misorientations'], d3d['mtr_caxis_misalignments'], d3d['mtr_solidity'], d3d['mtr_intensity'], d3d['mtr_aspect_ratios']]
                    new_raw_data = DataFrame(data, columns=['MTR Area, um^2', 'MTR Misorientation, deg', 'MTR Caxis Misalignment, deg', 'Solidity', 'MTR Intensity', 'MTR Aspect Ratio'])
                    ...
                    mtr_id_map_w_boundaries = (mark_boundaries(mtr_id_map, d3d['mtr_id_map'], color=(1,1,1), mode='inner') * 255).astype('uint8')
                    mtr_id_map_with_scalebar = add_scalebar(d3d=None, rgb_image=mtr_id_map_w_boundaries, stepsize = d3d['stepsize'], plot=False)
                    imsave(os.path.join(os.path.dirname(fid), 'Individual_MTRs.png'), np.array(mtr_id_map_with_scalebar))

                    for ref in ['x','y','z']:
                        ipf_with_scalebar = add_scalebar(d3d=None, rgb_image=d3d[f'ipf_cleaned_{ref.lower()}'], stepsize = d3d['stepsize'], plot=False)
                        imsave(os.path.join(os.path.dirname(fid), 'IPF_Images', ref.upper(), f'IPF_Cleaned_{ref.upper()}_Image_w_Scalebar.png'), ipf_with_scalebar)

                        #Generate MTR Only IPF Map and Save
                        mtr_ipf_with_scalebar = add_scalebar(d3d=None, rgb_image=d3d[f'ipf_mtr_{ref.lower()}'], stepsize = d3d['stepsize'], plot=False)
                        imsave(os.path.join(os.path.dirname(fid), 'IPF_Images', ref.upper(), f'IPF_MTR_{ref.upper()}_Image_w_Scalebar.png'), mtr_ipf_with_scalebar)
            ...
            if len(raw_data):
                ...
                raw_data = raw_data[['Sample', 'MTR Class', 'MTR Area, um^2', 'MTR Caxis Misalignment, deg', 'MTR Misorientation, deg', 'Solidity', 'MTR Intensity', 'MTR Aspect Ratio']]
                raw_data.to_csv(raw_data_output_path)

            else:
                response = messagebox.showwarning(title='Warning', message="No MTRs identified using current settings. Hit 'Yes' to Exit. Hit 'No' remain and adjust settings.", type='yesno')
                if response == 'yes':
                    self.quit()
    
            response = messagebox.showinfo(title='Microtexture Analysis Status', message="Program has completed successfully. Click 'Yes' to Exit or 'No' to remain.", type='yesno')
            if response == 'yes':
                self.quit()
...
class GenericPipelineBuilderUI(Frame):
    ...
    def initUI(self):
        ...
        frame1 = Frame(self)
        frame1.pack(fill=X)
        lbl1 = Label(frame1, text="Output Folder Name:", width=self.lw, anchor='e', justify=RIGHT, font=('Arial', '9','bold'), background='#f2f2f2')
        ...
        frame2 = Frame(self)
        frame2.pack(fill=X)
        lbl2 = Label(frame2, text="C-Axis Misalignment Threshold (deg)\n For Pixel Segmentation: ", width=self.lw, anchor='e', justify=RIGHT, font=('Arial', '9','bold'), background='#f2f2f2')
        lbl2.pack(side=LEFT, padx=5, pady=10)
        self.entry2 = Entry(frame2)
        ...
        lbl3_mask1_main = Label(frame3a, textvariable=self.mask_text, width=self.lw, anchor='e', justify=RIGHT, font=('Arial', '9','bold'), background='#f2f2f2')
        ...
        lbl_primary_cleanup = Label(frame4, textvariable=self.cleanup1_text, width=self.lw, anchor='e',font=('Arial', '9','bold'), justify=RIGHT, background='#f2f2f2')
        ...
        lbl_primary_cleanup_value = Label(frame4a, textvariable=self.primary_cleanup_text, width=self.lw, anchor='e', justify=RIGHT, font=('Arial', '9'))
        ...
        lbl_secondary_cleanup = Label(frame4b, textvariable=self.secondary_cleanup_header_text, width=self.lw, anchor='e',font=('Arial', '9','bold'), justify=RIGHT, background='#f2f2f2')
        ...
        lbl_secondary_cleanup_value = Label(frame4c, textvariable=self.secondary_cleanup_text, width=self.lw, anchor='e', font=('Arial', '9'))
        ...
        frame5 = Frame(self)
        frame5.pack(fill=X)
        lbl_mtr_size = Label(frame5, text="Minimum MTR Size, um^2:", width=self.lw, anchor='e', justify=RIGHT, font=('Arial', '9','bold'), background='#f2f2f2')
        ...
        frame6 = Frame(self)
        frame6.pack(fill=X)
        lbl_version = Label(frame6, text="Dream3D Version:", width=self.lw, anchor='e', justify=RIGHT, font=('Arial', '9','bold'), background='#f2f2f2')
        lbl_version.pack(side=LEFT, padx=5, pady=10)
        
        self.int_dream3d_version = IntVar()
        self.rbtn2 = Radiobutton(frame6, text="DREAM3D-6.5.49-Win64", variable=self.int_dream3d_version, value=0, command = partial(self.getDream3DVersionAndFileExtension, True))
        self.rbtn2.pack(fill=X, padx=10, expand=True)
        #========================================================================================================================

        frame7 = Frame(self)
        frame7.pack(fill=X, pady=10)
        lbl7 = Label(frame7, text="Overwrite Existing Files?", width=self.lw, anchor='e', justify=RIGHT, font=('Arial', '9','bold'), background='#f2f2f2')
        ...
        self.note = StringVar()
        self.note.set("* It is recommended that end users perform a study to establish appropriate threshold values for data cleaning.")
    ...
    def verifyFilePaths(self, file_paths):
        ...
        for fid in file_paths:
            ...
            if fid.find(' ') > 0: 
                response = messagebox.showwarning(title='Warning', message="Spaces were detected in the input file paths. Please ensure that all spaces are removed prior to running Dream3d.", type='ok')
            else:
    ...
    def onGenerateFiles(self):
        if hasattr(self, "file_paths") and hasattr(self, 'load_method'):
            if self.load_method == 'files':
                ...
                response = messagebox.showwarning(title='Warning', message="To re-generate input files, please select ANG or CTF files using the top left button. Then hit 'Generate Input File(s)'.", type='ok')
    ...
    def prepareInputs(self, files_already_exist=False):
        ...
        for subdir, oim_path in zip(subdirectories, self.file_paths):
            ...
            if self.int_overwrite.get() == 1 or not os.path.exists(os.path.join(parent_dir, subdir, subdir + file_ext).replace('\\', '/')):
    ...
    def generateDream3dInputs(self):
        ...
        response = messagebox_type(
            ...
            message = 
                f"{len(self.inputs['paths'].keys())} / {len(self.all_file_paths)} input files were successfully written, given the current overwrite settings.",
            type='ok')
    ...
    def onSubmit(self):
        ...
        if len(self.file_paths):
            ...
            if self.int_overwrite.get() == 1 or len(self.outputs):
                ...
                response = messagebox.showinfo(title='Status Update', message="Dream3D runs have completed. Would you like to exit the program?.", type='yesno')
                if response == 'yes':
                    self.quit()
            else:
                response = messagebox.showinfo(title='Dream3D Status', message=f"Given the current settings, the program has not submitted the requested Dream3D jobs as this process would overwrite existing files.", type='ok')

        #No input files chosen....     
        else:
            response = messagebox.showwarning(title='Warning', message="No input file(s) selected. Click YES to exit the program, or NO to keep interface open.", type='yesno')
            if response == 'yes':
                self.quit()
