...
def read_dream3d_file(d3d, ref_dir = [0,0,1], mtr_size = 10000):
    ...
    try:
        ...
        d['caxis_misalignments'] = calc_misalignment(d['raw_caxis'].reshape(-1,3), ref_dir = ref_dir).reshape(d['raw_caxis'].shape[:2])
    ...
    d['mtr_intensity'] = d['mtr_sizes'] * solidity * np.cos(d['mtr_caxis_misalignments'] * np.pi/180) / d['mtr_misorientations'] / 1e4
    ...
    dim1, dim2 = d['ipf_cleaned_z'].shape[0]*d['stepsize']/1000., d['ipf_cleaned_z'].shape[1]*d['stepsize']/1000.
